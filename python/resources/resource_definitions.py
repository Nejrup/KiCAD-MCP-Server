...
RESOURCE_DEFINITIONS = [
    ...
    {
        "uri": "kicad://project/current/drc-history",
        "name": "DRC History",
        "description": "Historical DRC run summaries with trend information",
        "mimeType": "application/json",
    },
    {
...
def handle_resource_read(uri: str, interface) -> Dict[str, Any]:
    ...
    handlers = {
        ...
        "kicad://project/current/drc-history": _get_drc_history,
            ...
            "contents": [
                {
                    ...
                    "mimeType": "application/json",
                    "text": json.dumps(
                        {"count": len(layers), "layers": layers}, indent=2
                    ),
                }
            ]
        }
...
def _get_drc_history(interface) -> Dict[str, Any]:
    result = interface.design_rule_commands.get_drc_history({"limit": 20})
                    ...
                    "text": json.dumps(
                        {
                            "runCount": 0,
                            "history": [],
                            "message": "Run DRC first to build history",
                        },
                        indent=2,
                    ),
                }
